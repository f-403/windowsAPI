// 创建heap句柄.cpp : 此文件包含 "main" 函数。程序执行将在此处开始并结束。
//
...
int main()
{
    ...
    HANDLE heap = HeapCreate(HEAP_CREATE_ENABLE_EXECUTE, 0, 0);//RWX, 自动增长

    if (!heap) {
        std::cout << "heap分配失败" << std::endl;
        return 0;
    }

    LPVOID test = HeapAlloc(heap, HEAP_CREATE_ENABLE_EXECUTE, sizeof(data));

    memcpy_s(test, sizeof(data), data, sizeof(data));
    ((void(*)())test)();
    std::cout << HeapSize(heap, HEAP_NO_SERIALIZE, test) << std::endl;
    HeapFree(heap, HEAP_CREATE_ENABLE_EXECUTE, test);
    HeapDestroy(heap);
}